...
extension UIImage {
   ...
   func brightnessStyleOfTopLeftImageContent(backColor: UIColor? = nil) -> ColorBrightnessStyle {
      let backColorBrightness: CGFloat = backColor?.brightness ?? 0.0

      let coef = size.width / size.height
      let resizeHeight = round(Self.resizeImageWidth / coef)
      let checkImage = self
         .cropped(rect: CGRect(x: 0, y: 0, width: size.width / 6, height: size.height / 2))
...
extension UIColor {
   ...
   func brightnessStyle() -> ColorBrightnessStyle {
      return brightness > ColorBrightnessStyle.brightnessTreshold ? .light : .dark
   }
}
